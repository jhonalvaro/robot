...
void SSD1306_DibujarPixel(uint8_t columna, uint8_t fila, _Bool negro);
void SSD1306_DibujarLinea(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, _Bool negro);
# 34 "main.c" 2
# 46 "main.c"
float theta0 = 90.0;
float theta1 = 90.0;
...
void controlServoMotor(int angulo) {
    ...
    valorPWM = (uint16_t)(((float)angulo / 180.0 * (7920 - 1520)) + 1520);
...
void setServoAngle(unsigned char servo, float angulo) {
    unsigned int pwmValue = (uint16_t)(((float)angulo / 180.0 * (7920 - 1520)) + 1520);
    switch (servo) {
        case 0:
